...
if numel(S) == 1
   ...
   % sections of code.
   SI = bwmorph(S, 'shrink', Inf);  % exemplo: se tiver 3 seed points ligados fica so com 1
   J = find(SI);
   S1 = f(J); % Array of seed values. % valor das seeds
end

% S1 e o valor das seeds na imagem original
% SI e uma imagem binaria com valor TRUE para as seeds (nunca aglomeradas)

TI = false(size(f));
for K = 1:length(S1) % para cada uma das seeds
   seedvalue = S1(K); % valor actual das seed
   S = abs(f - seedvalue) <= T; % acrescenta de acordo com o treshold
   TI = TI | S;
end
...
[g, NR] = bwlabel(imreconstruct(SI, TI)); % so cresce para regioes ligadas as seeds
